...
create or replace function prism_create_by_day(year int,fn text)
RETURNS integer
...
create index prism_by_day_east_north on prism_by_day(east,north);
create index prism_by_day_ymd on prism_by_day(ymd);
return count;
end
$$;

create or replace function prism_add_to_wy(year integer)
returns bigint as $$
insert into prism_wy(dwr_id,water_year,ymd,tn,tx,pcp,eto,rf)
 select
...
--select 1;
-- return new data
select count(*) from prism_wy where water_year=$1;
$$ LANGUAGE SQL VOLATILE;

create or replace function prism_add_wy(year integer,fn text)
returns bigint as $$
drop table prism_by_day;
select prism_create_by_day($1,$2);
select prism_add_to_wy($1);
$$ LANGUAGE SQL;

create or replace function prism_out_by_dwrid
...
create or replace function prism_out_wy
...
LOOP
    fn=base||'/'||dwr_id||'.csv';
    EXECUTE format('copy ('||
      'select * from dwr_out_by_dwrid(''%1$s'',%2$s))'||
