...
create or replace function cimis.create_by_day(year int,fn text)
RETURNS integer
...
create or replace function cimis.add_to_wy(year integer)
returns bigint as $$
insert into cimis.wy(dwr_id,water_year,ymd,tn,tx,tdew,u2,eto,k,rnl,rs)
 select
...
and c.water_year=$1;
-- return new data
select count(*) from cimis.wy where water_year=$1;
$$ LANGUAGE SQL VOLATILE;

create or replace function cimis.add_wy(year integer,fn text)
returns bigint as $$
drop table cimis_by_day;
select cimis.create_by_day($1,$2);
select cimis.add_to_wy($1);
$$ LANGUAGE SQL;

create or replace function cimis.out_by_dwrid
...
create or replace function cimis.out_wy
...
LOOP
    ...
    EXECUTE format(
    'copy ('||
    'select * from cimis.out_by_dwrid(''%1$s'',%2$s))'||
...
create or replace function cimis.et_out_by_dwrid
(in_dwr_id varchar(8),in_water_year integer)
RETURNS TABLE(
 X integer,Y integer,
...
create or replace function cimis.et_out_wy
  (base text, in_water_year integer)
RETURNS SETOF text
LANGUAGE PLPGSQL VOLATILE AS $$
declare
